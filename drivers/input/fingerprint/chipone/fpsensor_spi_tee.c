...
{
	printk("wingtech unblank begin\n");
	//mdss_prim_panel_fb_unblank(FP_UNLOCK_REJECTION_TIMEOUT);
	printk("wingtech unblank end\n");
}
...
static long fpsensor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FPSENSOR_IOC_RELEASE_VERSION: 
        ...
        if (copy_from_user(lib_ver_buf,((version_info_t *)arg)->ver_name, max_lib_buf)) {
            fpsensor_debug(ERR_LOG, "fpsensor Failed to copy information from user\n");
            retval = -EFAULT;
            break;
        }
        printk(KERN_INFO "%s: release version info: %s\n", __func__, lib_ver_buf);
        fpsensor_debug(ERR_LOG,"lib_ver_buf= %s\n", lib_ver_buf);
		if(!strcmp(lib_ver_buf,"chipone")){
			strcpy(fp_info,"chipone_ICNF7332AL");
			printk("fp_info[]=%s\n", fp_info);
		}
        break;
    default:
...
static unsigned int fpsensor_poll(struct file *filp, struct poll_table_struct *wait)
{
    ...
    if ( g_fpsensor->RcvIRQ) {
        if (g_fpsensor->RcvIRQ == 2) {
            ...
            ret |= POLLHUP;
        } else if (g_fpsensor->RcvIRQ ==3) {
            fpsensor_debug(ERR_LOG,"get fp off notify\n");
            ret |= POLLOUT;
        } else {
            fpsensor_debug(ERR_LOG, " get fingerdown irq\n");

			if ((fpsensor_dev->wait_finger_down == true) && (fpsensor_dev->device_available == 1) && (fpsensor_dev->fb_status == 0)) {
				fpsensor_dev->wait_finger_down = false;
				schedule_work(&fpsensor_dev->work);
...
static int fpsensor_fb_notifier_callback(struct notifier_block* self, unsigned long event, void* data)
{
    ...
    case FB_BLANK_POWERDOWN:
        ...
        fpsensor_dev->RcvIRQ = 3;
        fpsensor_dev->fb_status = 0;


        fpsensor_dev->wait_finger_down = true;
...
static int fpsensor_probe(struct platform_device *pdev)
{
    ...
    strcpy(fp_info,"sunwave_SW9058G");
    printk("fp_info[]=%s\n", fp_info);
	...
	fpsensor_dev->wait_finger_down = false;
	INIT_WORK(&fpsensor_dev->work, notification_work);
	printk("wingtech fast screen");
